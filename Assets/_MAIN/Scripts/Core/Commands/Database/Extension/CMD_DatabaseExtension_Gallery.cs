...
namespace COMMANDS
{
    ...
    public class CMD_DatabaseExtension_Gallery: CMD_DatabaseExtension
    {
        ...
        public static IEnumerator ShowGalleryImage(string[] data)
        {
            ...
            if (graphic == null)
            {
                Debug.LogError($"Could not find gallery image called '{mediaName}' in the Resources '{FilePaths.resources_gallery}' directory. Please specify the full path to the resources.");
                yield break;
            }
        ...
        public static IEnumerator HideGalleryImage(string[] data)
        {
            ...
            //Try to get the speed of the transition if it is not an immediate effect if (!immediate)
            parameters.TryGetValue(PARAM_SPEED, out transitionSpeed, defaultValue: 1);
